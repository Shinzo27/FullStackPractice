const BidTable = ({bids}: { bids: [string, string][] }) => {
    ...
    const bidWithTotal: [string, string, number][] = releventBids.map(([price, quantity]) => [price, quantity, currentTotal += Number(quantity)])
    const maxTotal = releventBids.reduce((acc, [_, quantity]) => acc + Number(quantity), 0)

    return (
        <div>
            {bidWithTotal?.map(([price,quantity, total]) => 
                <Bid maxTotal={maxTotal} price={price} quantity={quantity} total={total} key={price}/>
            )}
        </div>
    );
}
